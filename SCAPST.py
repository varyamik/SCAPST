...
st.markdown("""
    ...
    /* Выравнивание элементов внутри строки параметров по низу */
    .row-widget.stHorizontalBlock, div[data-testid="stHorizontalBlock"] {
    ...
    .coord-info {
        ...
        height: 40px;
        margin-top: 27px;
        font-size: clamp(12px, 1.1vw, 14px); /* Адаптивный размер шрифта */
        white-space: normal;                 /* Разрешаем перенос строк */
        word-break: break-word;              /* Переносим длинные слова и числа, если они не помещаются */
        overflow: hidden;                    /* Скрываем всё, что случайно вылезет за границы */
        box-sizing: border-box;              /* Учитываем отступы внутри ширины блока */
    }
...
@st.cache_data(show_spinner=False)  # Отключаем стандартный спиннер, так как используем свой прогресс-бар
...
def plot_cluster_diagrams(df_sel, radius_am, l_med, b_med):
    bg_color = 'None'
...
class ClusterAppUI:
    ...
    def render_interface(self):
        left_half, right_half = st.columns([1,1], gap="large")
        ...
        with left_half:
            ...
            if calc_clicked:
                ...
                if df is not None and not df.empty:
                    ...
                    fig1, fig2 = plot_cluster_diagrams(df_sel, p["radius_am"], l_med, b_med)
            ...
            if st.session_state.get("calculated", False) and st.session_state.get("results") is not None:
                ...
                df_sel_saved = res.get("df_sel")

                with st.container(border=True):
                    st.markdown("**Modes of Astrometric Parameters' Distributions**")
                    st1, st2, st3, st4 = st.columns(4)
                    st1.markdown(f"<div class='stat-card'><b> &mu;<sub>&alpha;</sub> = {res['c_mua']:.2f} mas/yr<br>V<sub>&alpha;</sub> = {res['v_a']:.2f} km/s </b></div>", unsafe_allow_html=True)
                    st2.markdown(f"<div class='stat-card'><b> &mu;<sub>&delta;</sub> = {res['c_mud']:.2f} mas/yr<br>V<sub>&delta;</sub> = {res['v_d']:.2f} km/s </b></div>", unsafe_allow_html=True)
                ...
                with st.container(border=True):
                    total_sources = len(df_sel_saved) if df_sel_saved is not None else 0
                    dbg_col1, dbg_col2, dbg_col3, dbg_col4 = st.columns(4)
                    dbg_col1.markdown(f"""<div class="stat-card" style="min-height: 120px;">
                    ...
                    dbg_col2.markdown(f"""<div class="stat-card" style="min-height: 120px;">
                                         <div style="font-weight: normal; font-size: 14px; color: #000000; min-height: 50px; align-items: center;">
                    ...
                    dbg_col3.markdown(f"""<div class="stat-card" style="min-height: 120px;">
                                         <div style="font-weight: normal; font-size: 14px; color: #000000; min-height: 50px; align-items: center;">
                    ...
                    dbg_col4.markdown(f"""<div class="stat-card" style="min-height: 120px;">
                                         <div style="font-weight: normal; font-size: 14px; color: #000000; min-height: 50px; align-items: center;">
                                         Nubmer of cluster stars, background-adjusted
                                         </div><div style="font-size: 28px; font-weight: normal; color: #000000;">
                                            {res['cluster_n']:.0f}
                                         </div>
                                         </div>""", unsafe_allow_html=True)
        ...
        with right_half:
            ...
            with st.container(border=True):
                ...
                if len(history_list) == 0:
                    ...
                    with col_cl:
                        if st.button("Clear Table History", key="clear_history_btn"):
                            st.session_state["cluster_history"][cluster_preset] = []
                            st.rerun()
